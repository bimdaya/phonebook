...
describe('contacts reducer', () => {
  ...
  it('should handle GET_CONTACTS_PENDING', () => {
    expect(getContactsReducer(initialState, {type: GET_CONTACTS_PENDING})).toEqual(
      {data: {}, fetching: true, fetched: false, failed: false}
    )
  })
  ...
  it('should handle GET_CONTACTS_FULFILLED', () => {
    ...
    expect(getContactsReducer(initialState, {
      type: GET_CONTACTS_FULFILLED,
      payload: data
    })).toEqual({data: data, fetching: false, fetched: true, failed: false})
  })

  it('should handle GET_CONTACTS_REJECTED', () => {
    expect(getContactsReducer(initialState, {type: GET_CONTACTS_REJECTED})).toEqual(
      {data: {}, fetching: false, fetched: false, failed: true}
    )
  })
})
